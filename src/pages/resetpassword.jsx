...
import { resetPassword } from '../https/resetpwd';
import { useSearchParams } from 'react-router-dom';
import { useState } from 'react';

function resetpassword() {
  const [setParams, setQueryParams] = useSearchParams({})
  const navigate = useNavigate()
  const consol = () => {
    console.log(setParams.toString())
    console.log(firstPwd)
    console.log(secondPwd)
  }
  const [firstPwd, setFirstPwd] = useState('')
  const onChangeFirstPwd = (e) => {
    e.preventDefault()
    setFirstPwd(e.target.value)
  }
  const [secondPwd, setSecondPwd] = useState('')
  const onChangeSecondtPwd = (e) => {
    e.preventDefault()
    setSecondPwd(e.target.value)
  }
  const [showPwd, setShowPwd] = useState(false)
  const setShowPwdHandler = () => {
  ...
  const submitReset = (e) => {
    ...
    return setPwdWrong(true);
    const body = {password_user: e.target.pwdOne.value}
    const url = import.meta.env.VITE_BACKEND_HOST + "/users/resetpassword?" + setParams.toString()
  ...
  return (
    ...
    <div className="flex h-screen">
      ...
      <form onSubmit={submitReset} className="flex-1 flex justify-center flex-col gap-y-5 px-2 md:px-10 desk:pr-def py-9">
        <div className="flex gap-2 items-center mb-4">
          ...
          <p onClick={consol} className='font-logo text-brown text-2xl'>
            Coffee Shop
          </p>
        </div>
        ...
        <div className="w-full border-2 border-solid border-order p-3 flex items-center gap-2 rounded-lg">
          ...
          <input onChange={onChangeFirstPwd} type={showPwd ? "text" : "password"} placeholder="Enter your password" name="pwdOne"  className='flex-1 outline-none  text-sm lg:text-base'/>          
          <div onClick={setShowPwdHandler}>
        ...
        <div className="w-full border-2 border-solid border-order p-3 flex items-center gap-2 rounded-lg">
          <ion-icon name="bag-outline"></ion-icon>
          <input onChange={onChangeSecondtPwd} type={showPwd ? "text" : "password"} placeholder="Enter your password" name="pwdTwo"  className='flex-1 outline-none  text-sm lg:text-base'/>          
          <div onClick={setShowPwdHandler}>
            <ion-icon name="eye-off-outline"></ion-icon>
          </div>
        </div>
        {pwdWrong && <p className='text-red-600'>Password Doesnt Match</p>}
        <div className="">
    ...
    {modals &&
      ...
      >
        ...
        >
          <p className="">Successfully change password</p>
